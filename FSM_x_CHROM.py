...
from FSM_for_validation import HRValidationFSM
...
def main(user_input=None):
    # creat an instance of our FSM
    hr_fsm = HRValidationFSM()
    BUFFER_SIZE = 300
    ...
    #  while camera.isOpened():
        while True:
            ...
            if L==300: # ถ้ามีเฟรมครบ 300 เฟรม
                ...
                # print(f'fps = {fps}')
                fft_of_interest, freqs_of_interest, rms = sp.normalized_fft(normalized_data, fps)
                print(f"freq = {fft_of_interest}")
              
                max_arg = np.argmax(fft_of_interest)
                bpm = freqs_of_interest[max_arg]
                CF = max_arg/rms
                print(f'({CF},{bpm})')
                current_state,recovery_count = hr_fsm.validate(bpm, CF)
                print(f'recovery_count = {recovery_count}')
                if current_state == 'stable':
                    cv2.putText(image, "HR: {}".format(round(bpm)), (int(image.shape[1]*0.5),int(image.shape[0]*0.95)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2)
                else:
                    cv2.putText(image, "HR: {}".format(current_state), (int(image.shape[1]*0.5),int(image.shape[0]*0.95)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2)
